...
public class ModRecipeProvider extends RecipeProvider {
	...
	protected void buildRecipes() {
		...
		this.shaped(RecipeCategory.MISC, ModBlocks.AUTO_SIEVE.get())
				.pattern("iri")
				.pattern("rsr")
				.pattern("iri")
				...
				.save(output);
		this.shaped(RecipeCategory.MISC, ModBlocks.AUTO_HAMMER.get())
				.pattern("iri")
				.pattern("rhr")
				.pattern("iri")
				.define('r', Tags.Items.DUSTS_REDSTONE)
		...
		this.shaped(RecipeCategory.MISC, ModBlocks.AUTO_SILKER.get())
				.pattern("iri")
				.pattern("rcr")
				.pattern("iri")
				.define('r', Tags.Items.DUSTS_REDSTONE)
		...
		this.shaped(RecipeCategory.MISC, ModItems.SPEED_UPGRADE.get())
				.pattern("iri")
				.pattern("rcr")
				.pattern("iri")
		...
		this.shaped(RecipeCategory.MISC, ModItems.REINFORCED_UPGRADE.get())
				.pattern("iri")
				.pattern("rcr")
				.pattern("iri")
		...
		this.shaped(RecipeCategory.MISC, ModItems.BONUS_UPGRADE.get())
				.pattern("iri")
				.pattern("rcr")
				.pattern("iri")
	...
	private void compressedBlockRecipes(DeferredBlock<CompressedBlock> block, Block compressed, RecipeOutput consumer) {
		...
		this.shaped(RecipeCategory.MISC, compressedBlock).pattern("aaa").pattern("aaa").pattern("aaa")
				.define('a', compressed)
				.unlockedBy("canCraft", InventoryChangeTrigger.TriggerInstance.hasItems(compressed))
				.save(consumer);
		this.shapeless(RecipeCategory.MISC, compressed, 9)
	...
	private void createCrushingRecipe(DeferredBlock<CompressedBlock> block, Block result, RecipeOutput output) {
		...
		builder.build(output, ResourceKey.create(Registries.RECIPE, modCrushingLoc(block.getId().getPath())));
	}

	private ResourceLocation modCrushingLoc(String s) {
		return ResourceLocation.fromNamespaceAndPath(ExNihiloAuto.MODID, "crushing/" + s);
	}
	...
	public static class Runner extends RecipeProvider.Runner {
		...
		@Override
		protected @Nonnull RecipeProvider createRecipeProvider(@Nonnull HolderLookup.Provider provider, @Nonnull RecipeOutput output) {
